...
export function Goals() {
  ...
  const getSkillLevelFeedback = (level: string, units: number) => {
    const metrics = SKILL_LEVEL_METRICS[level];
    if (!metrics) return '';

    if (units <= 8) {
      return `Based on your level (${metrics.label}), selling ${units} units this month is a great foundation to build on!`;
    } else if (units <= 15) {
      return `Based on your level (${metrics.label}), selling ${units} units this month is an exciting challenge within reach!`;
    } else {
      return `Based on your level (${metrics.label}), selling ${units} units this month is an ambitious goal - you've got this!`;
    }
  };
            ...
            <form onSubmit={handleSubmit(handleGoalSubmit)} className="space-y-6">
              ...
              <div className="text-sm text-earth-500 dark:text-earth-400 space-y-2">
                <p className="text-center">
                  {getSkillLevelFeedback(skillLevel, targetUnits)}
                </p>
              </div>
